...
if (isset($_POST['validSupp'])) {
	...
	$delete = sprintf("DELETE FROM uti_utilisateur WHERE uti_oid=%d ", $idUser);

	$validDel = $bdd->exec($delete);

	$valide = '<div class="text-success"> Votre compte à été supprimé avec succès </div>';
	header('refresh:3;url=?p=deco');
	$loader = "<div class='container'>
	<div class='row'>
	<div id='loading' class='loader'></div>
	</div>
